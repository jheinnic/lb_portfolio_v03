...
TICKET_MAX_FEATURES=64
TICKET_SIFT_OCTAVES=3
TICKET_SIFT_SIGMA=5.1735
...
TICKET_MIN_MATCH_PERCENT = 0.20
TICKET_MAX_DIST_RATIO = 0.8

DATA_DIR = 'data25'
EXEMPLAR_FILE_TEMPLATE = 'letters10/%s%02d.tif'
TICKET_LABEL_TEMPLATE = "(%d,%d) of %s"
...
class TicketCell:
   ...
   def identifyContent(self, exemplars, matchLog):
      ...
      percentiles = findPercentiles(blankTestImg, [0.15, 0.65])
      if (percentiles[0.15] + 6) >= percentiles[0.65]:
         self.contentString = '-'
         return '-'
...
XW_TASK_SIFT = cv2.xfeatures2d.SIFT_create(
   TICKET_MAX_FEATURES, TICKET_SIFT_OCTAVES, 0, 0, TICKET_SIFT_SIGMA)

class KnownSample(sampleDir):
   INSTANCE = new KnownSample('knownSample')

   def __init__(self, label, dataDir, siftInstance):
      self.label = label
      self.sampleDir = sampleDir
      self.imagePath = sampleDir + '/image.tif'
      self.queryImg = cv2.imread(self.imagePath, 0)
      self.imageData = imageData
      self.cornerCoordinates = np.load(sampleDir + '/corners.dat')
      for ii in range(0,NUM_CELLS)
         for jj in range(0,NUM_CELLS):
            locationRead = self.cornerLocations[ii][jj]
            self.cornerLocations[ii][jj] = locationRead + CELL_CORRECTION
   ...
   def validateFeatures(self):
      if self.keypoints is None or self.descriptors is None:
         throw Error('Keypoints and descriptors from SIFT must be non-empty arrays, not None.')
      self.featureCount = len(self.keypoints)
      if self.featureCount != len(self.descriptors):
         var message = 'fail with <%d> keypoints != <%d> descriptors' % (self.featureCount, len(self.descriptors))
         self.keypoints = None
      ...
      if self.featureCount <= 0:
         var message = 'fail with negative feature count = <%d> features' % (self.featureCount)
         self.keypoints = None
         self.descriptors = None
         self.featureCount = 0
         self.featuresReady = false
   ...
   def computeFeatures(self):
      if self.featuresReady:
         return
      var sifter = siftInstance || XW_TASK_SIFT
      (self.keypoints, self.descriptors) =
         sifter.detectAndCompute(self.imageData, None)
      self.validateFeatures();
      self.keypoints.dump(self.sampleDir + '/keypoints.dat')
      self.descriptors.dump(self.sampleDir + '/descriptors.dat')

   def loadFeatures(self):
      if self.featuresReady:
         return
      self.keypoints = np.load(self.sampleDir + '/keypoints.dat')
      self.descriptors = np.load(self.sampleDir + '/descriptors.dat')
      try:
          self.validateFeatures()
          return 
...
class CrosswordTicket:
   ...
   def locateCrossword(self, queryObj):
       ...
       (found, self.framePts, self.ticketPts, self.goodMatches) =
          matchQueryToTrain('ticket', kpQ, desQ, kpT, desT, GRID_MAX_DIST_RATIO, GRID_MIN_MATCH_PERCENT)
       if found == True:
   ...
   def processCrosswordGrid(self, sampleDataDir):
      ...
      for ii in range(0,NUM_CELLS):
         for jj in range(0,NUM_CELLS):
            ...
            xwGrid[ii][jj] = cellObject.identifyContent(CrosswordTicket.KNOWN_LETTERS, matchLog)
            xwStr += xwGrid[ii][jj] + ' '
         xwStr += "\n"
      print xwStr
