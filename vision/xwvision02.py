...
class KnownExample(SiftImage):
   def __init__(self):
      ...
      self.gridPoints = np.load(self.dataDir + '/grid.dat')
      for ii in range(0,NUM_CELLS):
         for jj in range(0,NUM_CELLS):
            self.cornerCoordinates[ii][jj] += CELL_CORRECTION
      top = max(self.gridPoints[1][0][1], self.gridPoints[2][0][1]) + 1
      bottom = min(self.gridPoints[0][0][1], self.gridPoints[3][0][1])
      right = max(self.gridPoints[2][0][0], self.gridPoints[3][0][0]) + 1
      left = min(self.gridPoints[0][0][0], self.gridPoints[1][0][0])
...
class MatchedPair:
   ...
   def processCrosswordGrid(self, visualizeCellClip=bool):
      if self.matchReady == False:
         ...
         if self.matchReady == False:
            ...
            return None
      ticketLabel = self.unknownTicket.getLabel()
      ...
      for ii in range(0,NUM_CELLS):
         for jj in range(0,NUM_CELLS):
            ...
            cellLabel = CELL_LABEL_TEMPLATE % (ii, jj, ticketLabel)
   ...
   def visualizeCrosswordLocation(self):
      ...
      knownGridPoints = self.knownExample.getKnownGridPoints()
      print(M)
      print(knownGridPoints)
      ...
      unknownPoints = cv2.perspectiveTransform(np.float32(knownGridPoints), M)
      unknownPoints.dump(self.dataDir + '/grid.dat')
      unknownPoints = np.int32(unknownPoints)
      decoratedImg = unknownImg.copy()
      decoratedImg = cv2.drawKeypoints(decoratedImg, kpT, None, color=(0,128,192)) #, flags=0)
      decoratedImg = cv2.polylines(decoratedImg, [unknownPoints], True, (160,0,0), 5, cv2.LINE_AA)
