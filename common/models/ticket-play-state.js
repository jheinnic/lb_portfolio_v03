(
  function () {
    ...
    module.exports = function (TicketPlayState) {

      TicketPlayState.getMultiplierCount = function getMultiplierCount() {
        // throw new Error('Abstract implementation');
        return 0;
      };

      TicketPlayState.getPrizeMatrix = function getPrizeMatrix() {
        return [[], []];
      };

      TicketPlayState.prototype.isDefinitionComplete = function isDefinitionComplete() {

      };

      TicketPlayState.prototype.revealNext = function revealNext(letter, cb) {
        if (this.yourLetters.contains(letter)) {
          cb(letter + ' has already been revealed');
        } else {
          this.yourLetters.replace('_', letter);
          this.save(cb);
        }
      };

      TicketPlayState.remoteMethod(
        'revealNext', {
          accepts: {
            arg: 'letter',
            required: true,
          ...
          // http: {path: '/stats'}
        }, TicketPlayState.prototype.stats
      );
      ...
      //
      //})();
      //TicketPlayState.prototype.analyzeProgress =
