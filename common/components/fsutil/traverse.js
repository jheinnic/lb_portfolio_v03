(function() {
  ...
  function traverseFileSystemSync(rootDirectory, fileCallback, dirCallback, isNormalized) {
    ...
    var _appendToPath;
    if (isNormalized) {
      _appendToPath = function _appendToPath(currentPath, dirItem) {
    ...
    if (dirCallback) {
      ...
      if (fileCallback) {
        _doTraversal(
          rootDirectory, _curryBothIterator(fileCallback, dirCallback, _appendToPath), _appendToPath
        );
      }
      else {
        _doTraversal(
          rootDirectory, _curryDirectoryIterator(dirCallback, _appendToPath), _appendToPath
        );
      }
      ...
      if (fileCallback) {
        _doTraversal(
          rootDirectory, _curryFileIterator(fileCallback, _appendToPath), _appendToPath
        );
      }
      else {
        console.log(
          'Saving time by skipping requested traversal without callbacks', rootDirectory
        );
      }
  }
  ...
  function _curryBothIterator(fileCallback, dirCallback, _appendToPath) {
    var _iterateNextEntry = function _iterateNextEntry(currentPath) {
      ...
      if (stats.isFile()) {
        ...
        if (stats.isDirectory()) {
          var test = dirCallback(currentPath, true);
          if (test) {
            _doTraversal(currentPath, _iterateNextEntry, _appendToPath);
          }
          dirCallback(currentPath, false);
  ...
  function _doTraversal(currentPath, _iterateNextEntry, _appendToPath) {
    fs.readdirSync(currentPath).filter(_filterSelfParent).map(
      function (dirItem) {
        return _appendToPath(currentPath, dirItem);
      }
