(function() {
  ...
  function buildNgAppBundle(env, angularScriptsDir, buildOutputDir, callback) {
    // TODO(bajtos) debug should be always true, the source maps should be saved to a standalone file when !isDev(env)
    // TODO: Resolve the fact that browserify's basedir is not currently defined using <%= appConfig.app %> because of
    //       apparent need for absolute path.
    var isDevEnv      = ['debug', 'development', 'test'].indexOf(env) >= 0,
        bundlePath    = path.join(buildOutputDir, 'client', 'app.js'),
        sourceMapPath = path.join(buildOutputDir, 'client', 'app.js.map'),
        sourceGlob    = path.join('**', '*.{js,coffee}'),
        // b             = browserify({basedir: path.join(__dirname, 'ngapp')}, {debug: isDevEnv});
        b             = browserify({basedir: angularScriptsDir}, {debug: isDevEnv});

    // Handle .coffee files
    b.transform('coffeeify');
    b._extensions.push('.coffee');

    // TODO: Deal with the hardcoding of 'jchptf' from application root module name here.
    ...
    try {
      boot.compileToBrowserify(angularScriptsDir, b);
    } catch (err) {
      console.error(err);
      return callback(err);
    }
